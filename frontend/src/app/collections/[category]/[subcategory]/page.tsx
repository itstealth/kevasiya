  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const { category: catSlug, subcategory: subCatSlug } = resolvedParams;
        setCategorySlug(catSlug);

        // Fetch subcategory info
        const apiUrl = getApiUrl();
        const catRes = await fetch(`${apiUrl}/categories?slug=${catSlug}`);
        if (!catRes.ok) {
        ...
        const prodRes = await fetch(
          `${apiUrl}/products?subcategory_id=${foundSubcategory.id}`
        );
        if (prodRes.ok) {
  ...
  return (
    <div className="bg-white">
      ...
      <div className="hidden sm:hidden">
        <WhatsAppCTA
          message={`Hello! I'm interested in your ${subcategoryName} collection. Can you help me?`}
        />
      </div>
